...
public partial class prg7013 : System.Web.UI.Page
{
        ...
        if (!emp.IsEmp) { Response.Redirect ("/Login.aspx"); }
            ...
            if (!emp.ChkPrg (PrgId)) { Response.Redirect ("/index.aspx"); }
                ...
                if (sDate != "" && eDate != "")
                    ...
                    DataTable dtlTbl = new DataTable ();
                    DataTable inStkTbl = new DataTable ();
                    if (dtl == "N")
                        {
                        if (outType == "0")
                            {
                            SqlComm = $"SELECT memSn, memName, SUM(outLeft) AS totalOutLeft, SUM(amount) AS totalAmount, SUM(amount) - SUM(outLeft) as pay  " +
                                $"FROM (SELECT memSn, memName, amount, outLeft FROM WP_vOut  " +
                                $"WHERE isDel = 'N' AND (OutStkDate BETWEEN  '{sDate}' AND '{eDate} 23:59:59') AND memSn NOT IN ('0', '-1') AND outType ='2' " +
                                $"GROUP BY memSn, memName, OutStkId, amount, outLeft) AS SubQuery " +
                                $"GROUP BY memSn,memName ORDER BY memSn";
                            name = "------已收";
                            ...
                            SqlComm = $"SELECT memSn, memName, SUM(outLeft) AS totalOutLeft, SUM(amount) AS totalAmount, SUM(amount) - SUM(outLeft) as pay  " +
                                $"FROM (SELECT memSn, memName, amount, outLeft FROM WP_vOut  " +
                                $"WHERE isDel = 'N' AND (OutStkDate BETWEEN  '{sDate}' AND '{eDate} 23:59:59') AND memSn NOT IN ('0', '-1') " +
                                $"GROUP BY memSn, memName, OutStkId, amount, outLeft) AS SubQuery  " +
                                $"GROUP BY memSn,memName ORDER BY memSn";
                            inStkTbl = getTbl.table ("WP", SqlComm);
                            }
                        ...
                        if (outType == "0")
                            {
                            SqlComm = $"SELECT * FROM " +
                                                    $"(SELECT memId, memName, OutStkId,  dtlOutLeft, MAX (amount) as amount, MAX (outLeft) as outLeft FROM WP_vOutStock " +
                                                    $"WHERE isDel = 'N' AND dtlIsDel = 'N' AND (OutStkDate BETWEEN '{sDate}' AND '{eDate} 23:59:59') AND memSn<>'0' " +
                                                    $"{(memSn == "" ? "" : $"AND memSn='{memSn}'")} " +
                                                    $"AND memId<> 'stkUpd' AND outType ='2' GROUP BY memId, memName, OutStkId, amount, dtlOutLeft) AS SubQuery ORDER BY memId";
                            inStkTbl = getTbl.table ("WP", SqlComm);
                            name = "------已收";
                            }
                        else if (outType == "1")
                    ...
                    Label2.Text += "<div class='search-sub'>" +
                         $"<span class='input-title'>明細</span>" +
                           ...
                           "</select>" +
                           "</div>";
                    SqlComm = $"SELECT * FROM WP_Member WHERE sn<>'0' ORDER BY memId";
                    ...
                    "</div>" +
                    "<div class='rptr-main'>" +
                        $"<div class='rptr-title'>應收帳款彙總表</div>" +
                        "<div class='rptr-data'>" +
               ...
               "<tr class='list-title'>" +
                   "<td class='align-l'>會員代號</td><td class='align-l'>會員名稱</td><td>應收金額</td><td>已結金額</td><td>未結金額</td>" +
               "</tr>";
                            Label2.Text += $"<tr><td colspan='5' class='empty-data'>查無資料！</td></tr>";
                            }
                        else
                            {
                      ...
                      "</tr>";
                            foreach (DataRow row in inStkTbl.Rows)
                                ...
                                if (((recNo) % 45) == 1 && recNo > 1)
                                    ...
                                    $"<div class='rptr-title'>應收帳款彙總表{name}</div>" +
                                    "<div class='rptr-data'>" +
                                        $"<div><div style='float:left;'>日期區間︰{sDate} 至 {eDate}</div><div style='float:right;'>頁次︰{pageNo} / <span class='page-total'></span></div></div>" +
                                        $"<div style='clear:both;text-align:right;width:100%;'>列印日期︰{DateTime.Today.ToString ("yyyy/MM/dd")}</div>" +
                                    "</div>" +
                        ...
                        if (inStkTbl.Rows.Count == 0)
                            ...
                            Label2.Text += "<table class='list-main' style='border-spacing:1px;'>" +
                          "<tr class='list-title'>" +
                              "<td class='align-l'>單號</td><td class='align-l'>會員名稱</td><td>應收金額</td><td>已結金額</td><td>未結金額</td>" +
                          "</tr>";
                            Label2.Text += $"<tr><td class='empty-data'>查無資料！</td></tr>";
                            }
                        else
                            {
                            ThreeDot threeDot = new ThreeDot ();
                            ...
                            foreach (DataRow row in inStkTbl.Rows)
                                ...
                                string Notpay = $"{row["dtlOutLeft"]:#0}";
                                string pay = $"{row["amount"]:#0}";
                                decimal totalAmount = Math.Round (Convert.ToDecimal (row["amount"]));
                                decimal pay2 = Math.Round (Convert.ToDecimal (row["dtlOutLeft"]));
                                decimal amount2 = totalAmount - pay2;
                                string amount = amount2.ToString ();
